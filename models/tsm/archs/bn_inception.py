...
class BNInception(nn.Module):
        ...
        if hasattr(self, 'is_temporal_shift') and self.is_temporal_shift[index] == '1':  # run temporal shuffling
            ...
            def wrapped_func(x, is_residual, n_segment, fold_div):
                if is_residual:
                    ...
                    try:
                        # TODO: Figure out the mix method of [original input] and [shifted input]
                        # return F.relu(x + layer_func(x_shift))
                        return F.relu(layer_func(x + x_shift))
                    except:
                        pdb.set_trace()
                else:
    ...
    def _build_features(self, inplace, num_classes):
        ...
        self.inception_4e_pool = nn.MaxPool2d((3, 3), padding=(1,0), stride=(2, 2), dilation=(1, 1), ceil_mode=True)
